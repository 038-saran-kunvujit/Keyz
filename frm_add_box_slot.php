     ...
     if (isset($_SESSION['complete'])) {
        if ($_SESSION['complete'] == "yes") {
          ...
          $i=1; // ลำดับจำนวนห้อง
           ...
           $result = $conn->query($sql);
?>
           <p class="text-center " style="font-size: 20px;">อพาร์ตเมนท์ : <?php $_SESSION['apartment_name'] ?></p>
           <p class="text-center " style="font-size: 20px;">เจ้าของอพาร์ตเมนท์ : <?php $_SESSION['username']?> </p><br>
           ...
           while ($row = $result -> fetch_assoc()) {
            ...
            <td><?php echo $i++ ?></td>
             <td><?php echo $row['box_id'] ?></td>
              <td><input type="text" class="appearance-none block w-full bg-[#eceff1] text-gray-700 border border-stone-950 rounded py-3 px-4 mb-2 mt-2  leading-tight focus:outline-none focus:bg-white"  name="room_number" value="<?php echo $row['room_number'] ?>"  ></td>

              <td><input type="text" class="appearance-none block w-full bg-[#eceff1] text-gray-700 border border-stone-950 rounded py-3 px-4 mb-2 mt-2 leading-tight focus:outline-none focus:bg-white"  name="slot_number" value="<?php echo $row['slot_number'] ?>"></td>

              <td><input type="text" class="appearance-none block w-full bg-[#eceff1] text-gray-700 border border-stone-950 rounded py-3 px-4 mb-2 mt-2 leading-tight focus:outline-none focus:bg-white"  name="rfid_number" value="<?php echo $row['rfid_number'] ?>"></td>
              <?php
            ...
            <?php  } ?>
               ...
               <button type="submit" name="send" class="mb-3 inline-block w-full rounded px-6 pb-2 pt-2.5 text-xs font-medium uppercase leading-normal text-white shadow-[0_4px_9px_-4px_rgba(0,0,0,0.2)] transition duration-150 ease-in-out hover:shadow-[0_8px_9px_-4px_rgba(0,0,0,0.1),0_4px_18px_0_rgba(0,0,0,0.2)] focus:shadow-[0_8px_9px_-4px_rgba(0,0,0,0.1),0_4px_18px_0_rgba(0,0,0,0.2)] focus:outline-none focus:ring-0 active:shadow-[0_8px_9px_-4px_rgba(0,0,0,0.1),0_4px_18px_0_rgba(0,0,0,0.2)]" style="background: linear-gradient(to right, #1A1A1A, #1A1A1A, #1A1A1A, #FF0000)">Submit</button>
               </div></td>
               </tr>
